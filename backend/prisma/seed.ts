...
async function main() {
  ...
  for (let r = 0; r < 6; r++) {
    ...
    if (r === 0 || r === 1) category = 'VIP';
    else if (r === 2 || r === 3) category = 'PREMIUM';
  ...
  const prices1: Record<string, number> = { VIP: 35, PREMIUM: 25, STANDARD: 18 };
  ...
  const prices2: Record<string, number> = { VIP: 120, PREMIUM: 75, STANDARD: 45 };
  await prisma.showSeat.createMany({
    data: v2Seats.map((vs) => ({
